...
To check for collinearity, we decided to make correlation plots for the continuous variables in the data set, and as can be seen in Figure 1, several variables are highly correlated since they are similar. including "college_jobs", "employed_fulltime", and "employed_parttime"; therefore, we transformed some of these variables into proportions such as "college_job_prop", which represents the proportion of jobs that requires a college degree. We believe these variables would be less correlated with other explanatory variables.

```{r data_cleaning, echo = F, fig.width=7, fig.height=4, fig.align='center'}
# load data
...
To evaluate the model's performance on predicting and avoid overfitting, we conducted a train-test split which takes 80% of the cleaned data as the training set.
...
The final model includes the following variables.(The whole variable selection process is described in "additional work")
...
Since displaying every step and the result in the variable selection process would be long and tedious, we have made plots that demonstrates the progression in variable selection, which provides a very brief outline regarding how we conducted variable selection.


```{r forward_backward_visualized, echo = F, fig.align='center',fig.width=7, fig.height=4}
par(mfrow=c(2,3))
...
We also created a summary table, and here is its description,
...
```{r diagnostics_plot, echo = F,fig.width=7, fig.height=4.5, fig.align="center"}
par(mfrow=c(2,2))
